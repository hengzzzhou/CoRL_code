"""
CORL Level 1 Simulation Reward - Updated to use CVSR Framework
"""
...
def compute_reward(solution_str: str, ground_truth: str, 
    ...
    """
    Compute reward for CORL Level 1 simulation tasks using CVSR framework.
    ...
    """
    # Map legacy bbox_reward_weight to CVSR weights
    if bbox_reward_weight > 0:
        # Training mode: emphasize grounding and spatial reasoning
        w_ground = min(0.5, bbox_reward_weight * 1.5)  # Scale up grounding importance
        w_overlap = 0.3
        w_ans = 1.0 - w_ground - w_overlap
        lambda1, lambda2 = 0.2, 0.8
    else:
        # Validation mode: emphasize final answer
        w_ground = 0.1
        w_overlap = 0.2  
        w_ans = 0.7
